...
const deleteContentItem = async (user, tableName, itemId) => {
  ...
  for (let field in tableData.fields) {
    ...
    if (val.type == 'Pointer' && val.targetClass == 'MediaItem') {
      ...
      if (media) media.destroy({ useMasterKey: true })
  ...
  if (itemDraft) {
    if (!checkRights(user, itemDraft)) throw 'Access denied!'
...
const deleteModel = async (
  ...
  if (!checkRights(user, model)) throw 'Access denied!'

  //removing model fields
  let fields = await getAllObjects(
    new Parse.Query('ModelField').equalTo('model', model)
  )
  ...
  if (deleteRef) {
    const models = await getAllObjects(
      new Parse.Query('Model').equalTo('site', model.get('site'))
    )
    ...
    for (let field of fields) {
      ...
      if (
        !validations ||
        !validations.models ||
        !validations.models.active ||
        !validations.models.modelsList
      )
        continue
...
Parse.Cloud.define('deleteContentItem', async (request) => {
  ...
  if (!tableName || !itemId) throw 'There is no tableName or itemId params!'
...
Parse.Cloud.beforeDelete(`Site`, async (request) => {
  ...
  if (!checkRights(request.user, site)) throw 'Access denied!'

  //removing site's models
  const models = await getAllObjects(
    new Parse.Query('Model').equalTo('site', site)
  )

  let promises = []
  for (let model of models)
    promises.push(promisifyW(deleteModel(request.user, model, false)))
  await Promise.all(promises)

  //removing site's collaborations
  const collabs = await getAllObjects(
    new Parse.Query('Collaboration').equalTo('site', site)
  )
...
const onCollaborationModify = async (collab, deleting = false) => {
  ...
  if (!collabACL) collabACL = new Parse.ACL(owner)
  ...
  for (let tempCollab of collabs) {
    if (tempCollab.id == collab.id) continue

    //set ACL for others collab
    let tempCollabACL = tempCollab.getACL()
    if (!tempCollabACL) tempCollabACL = new Parse.ACL(owner)
  ...
  if (!siteACL) siteACL = new Parse.ACL(owner)
  ...
  const mediaItems = await getAllObjects(
    new Parse.Query('MediaItem').equalTo('site', site)
  )

  for (let item of mediaItems) {
    let itemACL = item.getACL()
    if (!itemACL) itemACL = new Parse.ACL(owner)
  ...
  for (let model of models) {
    ...
    if (!modelACL) modelACL = new Parse.ACL(owner)
    ...
    getTableData(tableName).then((response) => {
      ...
      if (!deleting && (role == ROLE_ADMIN || role == ROLE_EDITOR)) {
        ...
        if (CLP['create'].hasOwnProperty(user.id)) delete CLP['create'][user.id]
        if (CLP['update'].hasOwnProperty(user.id)) delete CLP['update'][user.id]
        if (CLP['delete'].hasOwnProperty(user.id)) delete CLP['delete'][user.id]
      ...
      setTableData(tableName, data, 'PUT').catch(() =>
        setTableData(tableName, data, 'POST')
      )
    })
  }
  ...
  const fields = await getAllObjects(
    new Parse.Query('ModelField').containedIn('model', models)
  )

  for (let field of fields) {
    let fieldACL = field.getACL()
    if (!fieldACL) fieldACL = new Parse.ACL(owner)
...
Parse.Cloud.beforeSave('Collaboration', async (request) => {
  ...
  if (!checkRights(request.user, collab)) throw 'Access denied!'
...
Parse.Cloud.beforeSave('Site', async (request) => {
  ...
  if (request.object.id) return true

  const user = request.user
  if (!user) throw 'Must be signed in to save sites.'

  const payPlan = await getPayPlan(user)
  if (!payPlan) return true

  const sitesLimit = payPlan.get('limitSites')
  if (!sitesLimit) return true

  const sites = await new Parse.Query('Site')
    .equalTo('owner', user)
    .count({ useMasterKey: true })

  if (sites >= sitesLimit) throw `The user has exhausted their sites' limit!`
...
Parse.Cloud.beforeSave(`Model`, async (request) => {
  ...
  for (let collab of collabs) {
    ...
    if (role == ROLE_ADMIN || role == ROLE_EDITOR) writers.push(user.id)
  }
...
Parse.Cloud.define('onContentModify', async (request) => {
  ...
  if (!URL) return 'Warning! There is no content hook!'
...
Parse.Cloud.define('inviteUser', async (request) => {
  ...
  if (!email || !siteName) throw 'Email or siteName is empty!'
...
Parse.Cloud.define('checkPassword', (request) => {
  ...
  if (!password) throw 'There is no password param!'
